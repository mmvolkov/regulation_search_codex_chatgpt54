...
Поисковый API на FastAPI, реализующий гибридный поиск и генерацию ответов:

- [src/regulation_search/search_api.py](src/regulation_search/search_api.py)

Логика поиска:

- получает запрос пользователя;
- создает dense-embedding через OpenAI (`text-embedding-3-large`);
- создает sparse-вектор через BM25 (`fastembed`);
- выполняет гибридный поиск в `Qdrant` (prefetch dense + sparse, RRF fusion);
- генерирует сводный ответ через OpenAI Chat Completions на основе найденных фрагментов.

Основные endpoint:

- `GET /api/health`
- `POST /search` — принимает `query`, `top_k`, `model`, `temperature`, `response_length`, возвращает `answer`, `fragments`, `answer_found`
