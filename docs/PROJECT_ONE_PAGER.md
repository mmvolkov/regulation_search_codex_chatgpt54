# Помощник для сотрудников: краткое описание проекта

## Что это за проект

`Помощник для сотрудников` это внутренняя поисковая система по корпоративным регламентам в формате `DOCX`.
...
4. Пользователь задает вопрос на естественном языке, выбирает модель генерации ответа и режим полноты (S — краткий, M — стандартный, L — развёрнутый).
...
7. Интерфейс показывает ответ (длина зависит от выбранного режима), источник и подтверждающие цитаты.
...
- интерфейс поиска с выбором модели генерации ответа (GPT-4o mini / GPT OSS 120B) и режима полноты (S / M / L);
