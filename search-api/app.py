...
RESPONSE_LENGTH_PROMPT = """Температура генерации установлена на 0.1.

Параметр полноты ответа: {response_length}.

Адаптируй длину и степень детализации ответа в зависимости от выбранного пользователем параметра "Полнота ответа":
...
Во всех режимах сохраняй точность, ясность и полезность ответа."""
...
def build_answer_system_prompt(response_length: str) -> str:
    return (
        f"{SYSTEM_PROMPT}\n\n"
        f"{RESPONSE_LENGTH_PROMPT.format(response_length=response_length)}"
    )
